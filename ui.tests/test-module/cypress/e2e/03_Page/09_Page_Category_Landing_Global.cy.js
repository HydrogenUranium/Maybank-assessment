describe('Global Category Landing Page', () => {
  const pageUrls = [
    Cypress.env('AEM_PUBLISH_URL') + '/content/dhl/global/en-global/small-business-advice.html'
  ];
    ...
    viewports.forEach((viewport, vIndex) => {
      context(`Testing on ${viewport}`, () => {
        ...
        it('Test case', function () {
          ...
          cy.get(selectors.articleTeaser, { timeout: 10000 }).should('have.length', 4);
          ...
          cy.get(selectors.articleGridCategories).then(($el) => {
            expect($el[0].scrollWidth).to.be.at.least($el[0].clientWidth);
          });
