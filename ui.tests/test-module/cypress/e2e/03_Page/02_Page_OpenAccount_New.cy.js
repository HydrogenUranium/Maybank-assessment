describe('Open An Account page', () => {
  ...
  it('1.Verify the title contains the correct text "Open An Account"', () => {
    cy.get('#title-v2-acf2487a6a').should('exist')
  });

  it('2.Verify Marketo form is present', () => {
    cy.get(".marketoForm")
      .should("not.be.empty")
      .should("be.visible");
  });

  it('3.Verify if all fields are present', () => {
  ...
  it('4.Verify an error message is displayed for each field when the input is invalid (e.g., an email field, a phone number field with non-numeric characters)', () => {
  ...
  it('5.Verify the form submits successfully when all fields are filled out correctly', () => {
    const testText = 'THIS IS FOR TEST PLEASE IGNORE';
    cy.get('#LblisBusiness').click();
    cy.get('#suspectCompanyname').type(testText);
    cy.get('#FirstName').type(testText);
