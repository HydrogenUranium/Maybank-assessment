      ...
      context(`Testing on ${viewport}`, () => {
        ...
        it('Test Case', () => {
          ...
          if (viewport === 'macbook-11') {
            ...
            cy.get('.navigation-row__right .button:contains("Apply for a Business Account"):first')
              .should('be.visible')
              .click({ force: true });
            cy.url().should('include', `${Cypress.env('AEM_PUBLISH_URL')}/discover/en-sg/ship-now`);
            cy.visit(pageUrl);

            cy.get('.navigation-item:contains("Contact us"):first')
              .should('be.visible')
              .click({ force: true });
            cy.wait(2000);
          ...
          if (viewport !== 'macbook-11') {
            ...
            cy.get('.short-banners > .cta-banner-with-points-component > .banner > .banner__body > .banner__body__button')
              .click({ force: true });
            cy.url().should('include', `${Cypress.env('AEM_PUBLISH_URL')}/discover/en-sg/ship-now`);
            cy.visit(pageUrl);
          ...
          cy.get('.headerV2__desktopCountry > .fi').click({ force: true });
