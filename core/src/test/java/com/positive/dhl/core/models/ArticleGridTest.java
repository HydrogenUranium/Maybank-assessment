...
class ArticleGridTest {
	...
	void test() {
		...
		assertEquals("latest", articleGrid.getMode());
		assertEquals(3, articleGrid.getCategories().size());
		assertEquals(0, articleGrid.getArticles().size());

		articleGrid.setMode("");
		articleGrid.setCategories(new ArrayList<CategoryLink>());
		articleGrid.setArticles(new ArrayList<Article>());

		assertEquals("", articleGrid.getMode());
		assertEquals(0, articleGrid.getCategories().size());
		assertEquals(0, articleGrid.getArticles().size());
	}
}