...
export const getPathPrefix = (): srting => {
...
export const getArticles = async (query: string): Promise<Article[]> => {
    ...
    return get({ url: `${prefix}${componentPath}.searcharticlesuggest.json?s=${query}&homepagepath=${homePagePathSuffix}` })
};

export const getTags = async (query: string): Promise<string[]> => {
    const prefix = getPathPrefix();
