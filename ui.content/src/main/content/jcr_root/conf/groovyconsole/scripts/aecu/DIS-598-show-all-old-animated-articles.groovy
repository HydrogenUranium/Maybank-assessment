...
templateToArticlesMap.each { template, articles ->
    articles.each{
        def page =it.replaceAll("/jcr:content", "")
        def node =  getNode(it)
        def status = node.hasProperty("cq:lastReplicationAction") ? node.getProperty("cq:lastReplicationAction").getString() : "not published"
        println("""${page} ${status}""")
    }
}
