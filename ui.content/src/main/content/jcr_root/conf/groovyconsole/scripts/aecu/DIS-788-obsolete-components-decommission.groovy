/* DIS-788 Obsolete Components Decommission
Steps:
0)  INITIAL SETUP
    - define scope in the @Field contentScope
    - define list of resource types in the @Field components
...
@Field components = [
        "dhl/components/content/article-author-panel",
        "dhl/components/content/article-content-banner",
        "dhl/components/content/article-hero-image",
        "dhl/components/content/article-similar-stories",
        "dhl/components/content/article-social-static",
        "dhl/components/content/articlelistcarousel",
        "dhl/components/content/articlelistcategoryexplore",
        "dhl/components/content/articlelistcategorytrending",
        "dhl/components/content/articlelisthomepage",
        "dhl/components/content/categorylist",
        "dhl/components/content/ctapanelrow",
        "dhl/components/content/homepagecarousel",
        "dhl/components/content/inlineshipnow",
        "dhl/components/content/inlineshipnowmarketo",
        "dhl/components/content/inlineshipnowtwo",
        "dhl/components/content/pagenotfound",
        "dhl/components/content/stats",
        "dhl/components/content/taglist",
]
@Field versionAndPackageName = "DIS-788-obsolete-components-decommission"
...
def getAffectedItemMap() {
    ...
    getPage(contentScope).recurse { page ->
        if (page.path ==~ /^\/content\/dhl\/(language-masters|global|\w{2})(\/.*)?/) {
            ...
            if (contentPage) {
                getNode(contentPage.path).recurse { node ->
                    if (node?.hasProperty('sling:resourceType')) {
                        def resType = node.getProperty('sling:resourceType').getString()
                        if (resType) {
                            if (components.contains(resType)) {
                                listForTable.add([node.path, resType])
                            }
                        }
                    }
                }
    ...
    if (showTable) {
        ...
        if (affectedItemPaths.size() > 0) {
            components.each{ resType ->
                ...
                println(">>> $resType: $iterator hits")
            }
            println("----------------------------------------")
            println "Component path,Component Resource Type"
            affectedItemMap.each({ println("""${it.join(',')}""") })
        }
    }
}
