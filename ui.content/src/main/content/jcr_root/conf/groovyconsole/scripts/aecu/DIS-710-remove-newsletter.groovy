/* DIS-710 Removing Newsletter
Steps:
0)  INITIAL SETUP
    - define scope in the @Field contentScope
    - define component name in the @Field componentNameForDeletion
    - define component property name in the @Field componentNameForDeletion
    - define component property value in the @Field componentPropertyValue
...
@Field componentNameForDeletion = "dhl/components/content/cta-banner-with-points"
@Field componentPropertyName = "type"
@Field componentPropertyValue = "subscribeNewsletter"

@Field affectedPagePaths = [

]

@Field versionAndPackageName = "DIS-710-before-removing-newsletter"
...
def main() {
    ...
    if (contentManipulation) {
        contentManipulation(affectedPagePaths)
    } else {
        if (dryRun) {
                ...
                if (getResource(page.path + "/jcr:content")) {
                    getNode(page.path + "/jcr:content").recurse { node ->
                        if (node?.hasProperty('sling:resourceType')) {
                            ...
                            if (resType) {
                                if (resType == "dhl/components/content/subscribepanel") {
                                    paths.add(page.path)
                                }
                            }
                        }
...
def contentManipulation(affectedNodePaths) {
    affectedNodePaths.each { pagePath ->
        ...
        aecu.contentUpgradeBuilder()
                ...
                .filterByProperty("sling:resourceType", "dhl/components/content/subscribepanel")
                .doDeleteResource()
                .run(dryRun)
