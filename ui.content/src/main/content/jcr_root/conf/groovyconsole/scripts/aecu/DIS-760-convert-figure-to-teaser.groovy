/* DIS-760 Convert Image (aka Figure) component to Teaser
Steps:
0)  INITIAL SETUP
...
@Field oldComponentResType = "dhl/components/content/figure"
@Field newComponentResType = "dhl/components/content/teaser"

@Field versionAndPackageName = "DIS-760-before-converting-figure-to-teaser"
...
def manipulations(affectedItemPaths) {
    affectedItemPaths.each { itemPath ->
        def node = getNode(itemPath)

        def alignmentValue = node.get('alignment')?.toString()
        def styleIdValue = (alignmentValue == "left") ? "1723043113061" :
                (alignmentValue == "right") ? "1723043139831" :
                        (alignmentValue == "full") ? "1723043149524" : ""

        def caption1 = node.get('caption1')?.toString()
        def caption2 = node.get('caption2')?.toString()
        def content = node.get('content')?.toString()
        def imageAltTextValue = (caption1) ? caption1 :
                (caption2) ? caption2 :
                        (content) ? content : "Image Alt Text Placeholder"
        ...
        aecu.contentUpgradeBuilder()
                ...
                .doRenameProperty("caption1", "jcr:title")
                .doRenameProperty("caption2", "pretitle")
                .doRenameProperty("content", "jcr:description")
                .doRenameProperty("imagernd", "fileReference")
                .doSetProperty("cq:styleIds", styleIdValue)
                .doDeleteProperty("alignment")
                .doSetProperty("imageFromPageImage", "false")
                .doSetProperty("altValueFromDAM", "true")
                .doSetProperty("alt", imageAltTextValue)
                .doSetProperty("isDecorative", "false")
                .doSetProperty("titleFromPage", "false")
                .doSetProperty("descriptionFromPage", "false")
                .run(dryRun)
    }
}
