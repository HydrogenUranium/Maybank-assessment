...
@Field versionAndPackageName = "DIS-737-before-unpublishing"
    ...
    if (affectedItemPaths.size() == 0) {
        getPage(contentScope).recurse { page ->
            ...
            if (pagePath ==~ /^\/content\/dhl\/(language-masters|global|\w{2})(\/.*)?/
                    ...
                    && contentPage
                    && contentPage.get('cq:lastReplicationAction') != 'Deactivate') {
...
def contentManipulation(affectedPagePaths) {
