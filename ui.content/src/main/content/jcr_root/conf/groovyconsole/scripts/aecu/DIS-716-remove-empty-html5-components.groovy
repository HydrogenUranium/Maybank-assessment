/* DIS-716 Removing empty HTML5 Embed components
Steps:
...
@Field versionAndPackageName = "DIS-716-before-removing-empty-html5-component"
...
def main() {
    ...
    if (contentManipulation) {
        ...
        removingHtml5Components(affectedItemPaths)
...
def getAffectedItemPaths() {
    ...
    if (affectedItemPaths.size() == 0) {
        sql2Query("""
            ...
            WHERE ISDESCENDANTNODE([$contentScope]) 
            AND (node.[sling:resourceType]="dhl/components/content/html5embed") 
            AND (node.[text] IS NULL OR node.[text] = '')
            """).each { paths.add(it.path) }

        return paths.sort().findAll{!it.startsWith("/content/dhl/Archive")}
    } else {
        return affectedItemPaths
    }
}

def removingHtml5Components(affectedNodePaths) {
