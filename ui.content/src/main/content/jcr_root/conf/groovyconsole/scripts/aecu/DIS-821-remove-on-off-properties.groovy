...
@Field versionAndPackageName = "DIS-821-before-remove-on-off-properties"
...
def main() {
    ...
    if (contentManipulation) {
        ...
        publishingAffectedPages(affectedItemPaths)
    } else {
        if (dryRun) {
...
def getAffectedItemPaths() {
    ...
    if (affectedItemPaths.size() == 0) {
        getPage(contentScope).recurse { page ->
            ...
            if (content && (content.get("offTime") || content.get("onTime")) && page.path ==~ /^\/content\/dhl\/(language-masters|global|\w{2})(\/.*)?/) {
                paths.add(page.path)
            }
        }
...
def manipulations(affectedItemPaths) {
    println "Running manipulations method"
    affectedItemPaths.each { itemPath ->
        aecu.contentUpgradeBuilder()
                .forResources((String[]) [itemPath + '/jcr:content'])
                .doDeleteProperty("offTime")
                .doDeleteProperty("onTime")
                .run(dryRun)
    }
}
...
def publishingAffectedPages(affectedPagePaths) {
    println "Running publishingAffectedPages"
